...
class GraspEnvUR5e:
    ...
    def reset_idx(self, envs_idx):
        ...
        random_z = torch.ones(num_reset, device=self.device) * 0.025  # 0.025
    ...
    def _reward_keypoints(self):
        ...
        # For UR5e, we use the ee tip position directly
        ee_pos_keypoints = self._to_world_frame(
            self.robot.ee_pose[:, :3],
            self.robot.ee_pose[:, 3:7],
            keypoints_offset,
        )
        object_pos_keypoints = self._to_world_frame(self.object.get_pos(), self.object.get_quat(), keypoints_offset)
        dist = torch.norm(ee_pos_keypoints - object_pos_keypoints, p=2, dim=-1).sum(-1)
        return torch.exp(-dist)

    def _reward_table_contact(self):
        # For UR5e without gripper, we can check if the end-effector is too low (indicating table contact)
        ee_height = self.robot.ee_pose[:, 2]
        table_height = 0.0
        contact_threshold = 0.05  # 5cm above table
        
        # Penalize when end-effector is too close to table
        contact_penalty = torch.where(
            ee_height < table_height + contact_threshold,
            -(table_height + contact_threshold - ee_height),
    ...
    def grasp_and_lift_demo(self, render: bool = False):
        ...
        for i in range(total_steps):
            if render:
                self.cam.render()
            if i < total_steps / 4:  # approaching
                ...
                self.robot.go_to_goal(reset_pose)
            self.scene.step()


## ------------ robot ----------------
class ManipulatorUR5e:
    ...
    def set_pd_gains(self):
        # set control gains for UR5e
        # These values are tuned for UR5e robot
        self._robot_entity.set_dofs_kp(
            torch.tensor([4000, 4000, 4000, 2000, 2000, 2000]),
        )
        self._robot_entity.set_dofs_kv(
            torch.tensor([400, 400, 400, 200, 200, 200]),
        )
        self._robot_entity.set_dofs_force_range(
            torch.tensor([-150, -150, -150, -28, -28, -28]),
            torch.tensor([150, 150, 150, 28, 28, 28]),
        )

    def _init(self):
        self._arm_dof_dim = self._robot_entity.n_dofs  # UR5e has 6 DOFs total (no gripper)
    ...
    def go_to_goal(self, goal_pose: torch.Tensor):
        q_pos = self._robot_entity.inverse_kinematics(
            link=self._ee_link,
            pos=goal_pose[:, :3],
            quat=goal_pose[:, 3:7],
